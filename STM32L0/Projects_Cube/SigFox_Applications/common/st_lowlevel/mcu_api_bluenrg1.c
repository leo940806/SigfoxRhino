...
#define MCU_API_VER		"v2.4.2"
#define MCU_API_VER_SIZE	      6

/** @}*/
volatile uint8_t spi_in_use=0, rtc_irq=0, rtc_in_use=0, rtc_in_use_for_cs=0, carrier_sense_tim_started=0;
...
{
 /* if the MCU is stuck waiting for some IRQ, keep on the BLE activity */
  BLE_Activity();  
} 
...
sfx_u8 MCU_API_free(sfx_u8 *ptr)
{
  ...
  // calle donly when SIGFOX_API_close() is called.
...
{
  PRINTF("MCU_API_timer_start_carrier_sense IN\n\r");
...
{
  (*size) = MCU_API_VER_SIZE;
  
  for(uint8_t i =0; i<MCU_API_VER_SIZE; i++)
    (*version)[i] = MCU_API_VER[i];
...
{ 
  ...
  S2LPEvalGpioInit((M2SGpioPin)pin, M2S_MODE_EXTI_IN);
  S2LPEvalGpioInterruptCmd((M2SGpioPin)pin, 3, 0, (FunctionalState)new_state);
  if(trigger)
  {
    S2LPEvalGpioTriggerRising((M2SGpioPin)pin, ENABLE);
  }
}
...
void ST_MCU_API_Shutdown(sfx_u8 value)
{
  ...
  /* wait some ms to wait the device to be ready */
  Clock_Wait(3);
...
void ST_MCU_API_SpiRaw(uint8_t n_bytes, uint8_t* in_buffer, uint8_t* out_buffer, uint8_t can_return_bef_tx)
{
  ...
  if((((uint32_t)in_buffer)&0x20000000)!=0x20000000)
  {
    ...
    S2LPEvalSpiRaw(n_bytes, aux_buff, out_buffer, can_return_bef_tx);
  }
  else
  {
    /* call the SPI function of the S2LP_EVAL module perform a generic SPI transaction */
    S2LPEvalSpiRaw(n_bytes, in_buffer, out_buffer, can_return_bef_tx);
  }
}
...
{
  S2LPEvalGpioInit ((M2SGpioPin) pin, M2S_MODE_GPIO_IN);
}


//To be fixed, based on gpio selected pin
