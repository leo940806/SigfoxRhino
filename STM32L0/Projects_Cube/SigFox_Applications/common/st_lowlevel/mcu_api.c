...
static uint8_t s2lp_irq_raised=0;
static TIM_HandleTypeDef  Tim4_Handler={.Instance=TIM4};
static uint8_t _encryptedPayload = 0;
static const uint8_t _libVersion[] = MCU_API_VER;
uint32_t led_nowtime;
uint32_t bat_tx_raw;
uint8_t bat_count = 0;
extern uint32_t starttime, bat_idle;
extern _Bool boot_flag;
...
////	
////	GPIO_InitStructure.Pin = ;// SDN , CS S2-LP and CS E2PROM
...
//		static int led_count = 0x00;
		uint16_t led_delay= 0x00; 
//		HAL_PWR_EnterSTOPMode(PWR_LOWPOWERREGULATOR_ON,PWR_STOPENTRY_WFI);   //Issue on EP2 PCB, the MCU does not wakeup during transmission: Soln -> this line is commented -> the current of the MCU is insignificant
//    
    ST_MCU_API_SetSysClock();
		...
		else if(s_state == S_SETUP)
		{
			led_delay = 75;
		}
		
		led_nowtime ++;						//Issue on EP2 PCB, Core system tick issue, HAL_Delay() is not stable when transmission: Soln -> Count with led_nowtime ++

			if( ((tracker_data.led_active == 1) && (s_state == S_SETUP)) || s_state == S_BOOT)
			{
				if(led_nowtime - starttime > led_delay*100)
				{
					starttime = led_nowtime;
...
//    setGpioRestore();
  ...
  if(s2lp_irq_raised)
  {
		...
		if(bat_count == 50)
		{
			HAL_ADC_Start(&hadc);															//40us
			HAL_ADC_PollForConversion(&hadc, 10);							//8us
			bat_tx_raw = get_bat_adc();
//			bat_tx_raw = get_bat();

			bat_count++;
		}
		else if(bat_count < 50)
		{
			bat_count ++;
		}
		#ifdef INCLUDE_WWDG
...
{
  TIM_HandleTypeDef  Tim4_Handler={.Instance=TIM4};
  uint16_t c;
  ...
  SdkEvalTimersTimConfig(&Tim4_Handler,16000-1,65535-1); // prescalar = 16000 - 1 originally
  __HAL_TIM_DISABLE_IT(&Tim4_Handler, TIM_IT_UPDATE);
...
void TIM4_IRQHandler(void)
{
  ...
  if(__HAL_TIM_GET_IT_SOURCE(&Tim4_Handler, TIM_IT_UPDATE) !=RESET) 
  {
    ...
    SdkEvalTimersState(&Tim4_Handler, DISABLE);
  }
}
				       ...
				       sfx_s16 *temperature)
{
  ...
  (*voltage_idle)=bat_idle;
  (*voltage_tx)=bat_tx_raw*2*VDD/4096 - BATADC_OFFSET;
  (*temperature)=(int32_t)get_temp();
...
sfx_u8 MCU_API_delay(sfx_delay_t delay_type)
{
  ...
  case SFX_DLY_CS_SLEEP:
    ...
    */
    priv_ST_MCU_API_delay(465-2*ST_RF_API_get_ramp_duration());
    break;
...
sfx_u8 MCU_API_timer_start_carrier_sense(sfx_u16 time_duration_in_ms)
{
  ...
  if(rtc_in_use)
  {
    ...
    SdkEvalTimersTimConfig(&Tim4_Handler,a-1,b-1);
    SdkEvalTimersState(&Tim4_Handler, ENABLE);
  }
  else
  {
    Configure_RTC_Clock();
...
sfx_u8 MCU_API_timer_start(sfx_u32 time_duration_in_s)
{
  ...
  uint32_t rtc_wup_tick, next_rtc_wakeup_tick;
//----------------my code--------------------
//	if(time_duration_in_s == 0x14)
//	{
//		time_duration_in_s -= 10;
//	}
//	time_duration_in_s = time_duration_in_s/2;
//----------------my code--------------------
...
sfx_u8 MCU_API_timer_wait_for_end(void)
{
  ...
  while(!rtc_irq)//
//	while(!(next_rtc_wakeup==0 || rtc_irq==1))
  {
		#ifdef INCLUDE_WWDG
		HAL_WWDG_Refresh(&hwwdg);
		#endif
