...
if (S2LPEvalGetVersion()==0 | S2LPEvalGetVersion()==1)
  {
    ...
    else if (GPIO_GetITPendingBit(GPIO_Pin_5))
    {
       Appli_Exti_CB(5);
  ...
  else if (S2LPEvalGetVersion()==2)
  {
    if(GPIO_GetITPendingBit(GPIO_Pin_21))
    {
      ST_RF_API_S2LP_IRQ_CB();

      GPIO_ClearITPendingBit(GPIO_Pin_21);
    }
  }
  //else
  //{
  //  Appli_Exti_CB(0);//Removed from the main
  //}
}
/**
  * @}
